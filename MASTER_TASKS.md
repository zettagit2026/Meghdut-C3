...
- 🔄 **JAM EFFECTIVENESS (`8480469`)** — per commander live-test (signal present, drone not stopped): removed the artificial auto-stop DURATION/REPEAT caps → **continuous + full-band SWEEP + max gain**; kill-switch PROVEN intact (every loop polls stop_event+tx_halt; tx_halt/abort/Stand-Down/arm/confirm/range-auth/930c-pin/IFF all UNCHANGED; 153+75 tests). **PL-011 controlled-landing 30s cap deliberately KEPT** (bounded landing maneuver, not an ineffective throttle) — user-flag if they want it unbounded. **Under security-review + verify → then deploy.**
