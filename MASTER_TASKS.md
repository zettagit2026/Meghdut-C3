...
- 🧱 **DJI DroneID decoder tree** — the ONLY protocol-library gap: `droneid` bridge is OFFLINE on .186 because `DRONEID_SRC_DIR=/CEMA/DroneSecurity/src` is set but that dir doesn't exist. Place the DroneSecurity/DJI-DroneID decoder source there (or re-point) → droneid promotes to READY. Honest OFFLINE, not fake. (source the decoder — check zettagit repos / OSS DJI-DroneID projects.)
