...
- 🧑 **DJI DroneID decoder tree — DECISION (licensing)** — the ONLY protocol-library gap: `droneid` OFFLINE because `DRONEID_SRC_DIR=/CEMA/DroneSecurity/src` doesn't exist on .186. Decoder = **RUB-SysSec/DroneSecurity, AGPLv3**; bridge imports it at runtime UNMODIFIED (not vendored/forked — clean). BUT AGPLv3 network-copyleft on an Army-fielded system is the same class the user swapped OUT on BEL (MinIO→SeaweedFS). **User call before placing it on the box.** If approved: checkout DroneSecurity so `src/` is at `/CEMA/DroneSecurity/src` (unmodified) → droneid → READY. Honest OFFLINE meanwhile, not fake.
...
## Shipped (verified)
- **Protocol library DEPLOYED to .186 (`cad26af`, 2026-09-04, verified GO + honest board):** over-the-air decoder bridges live — **control_link = LIVE** (real: 895 decodes / 500 contacts classified off the detection plane), remoteid + fpv_osd = READY (feeds up, honest — no drone/video present), droneid = OFFLINE (missing decoder tree, honest not fake). 331 files sha256-verified; backend+frontend REBUILT (baked-image trap avoided); OPERATIONAL(×4)/FORENSIC(×4) grouping. DroneID cued (not blind-sweep) + PROVEN not to starve the RX radio. End-state clean: `tx_halted=True`, TX bridges dormant+disabled, detections incrementing.
