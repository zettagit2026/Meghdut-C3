...
def _post_with_reauth(console_url: str, path: str, json_body: dict, headers: dict,
    ...
    headers.setdefault("X-Bridge-Name", "ltm_parser")
...
class LTMSerialBridge:
    ...
    def __init__(self, console_url: str, headers: dict, serial_device: str,
                 baud: int = LTM_DEFAULT_BAUD, repost_interval_s: float = 10.0,
    ...
    def run_forever(self) -> None:
        ...
        # LTM traffic is on the wire -- print a cheap "still listening" line
        ...
        while True:
            ...
            if not chunk:
                ...
                if now_idle - last_idle_heartbeat >= IDLE_HEARTBEAT_INTERVAL_S:
                    ...
                    print(f"[ltm_parser] [heartbeat] still listening on "
                          f"{self.serial_device} -- no LTM traffic decoded in "
